...
def main(args):
    ...
    mcnntemplate_number=condor_queue=len(FxFx_lhe_files)
...
'''
    ...
    print('------- ADD the LHE file fot FxFx ----------\n')
    for i,item in enumerate(FxFx_lhe_files):
        i=str(i).zfill(4)
        name=f'{output_path}/{i}/rivet{name}_cfg.py'
        cmd=f'''sed -i "/Beams:frameType/a \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ 'Beams:LHEF = {item}'," {name}'''
        print(cmd)
        os.system(cmd)
...
sed -i 's/initialSeed = 1/initialSeed = {rnd}/"' {output_path}/${{i}}/rivet{name}_cfg.py
