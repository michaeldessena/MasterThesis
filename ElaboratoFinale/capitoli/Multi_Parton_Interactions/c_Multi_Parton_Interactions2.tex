...
\\
The parton shower consists of two components: the initial-state radiation (ISR) describing the emission from the incoming partons and the final-state radiation (FSR) describing the emission of outgoing partons they are also respectively referred to as spacelike and timelike showers.
...
Instead, \eqRef{eq:ISR1} refers to the same probability but it includes the description of the PDF evolution in fact the ISR is described in terms of production of a parton (instead of decays probability as in FSR). The slitting kernels are the same described above except that the production of a gluon $P_g(z)$ is equal to $2P_{gg}(z)$ since two gluons are generated from the decay of one gluon.

Staring from these infinitesimal probabilities by multiplication of no-emission probability $1-dP_{ISR}$, or equally $1-dP_{ISR}$. 
...
\end{equation}
so, in the two cases: for the FSR $Q^2_{FSR}=(p^2-m_0^2)$ a time-like virtuality ($p^2>0$) is implicated, while for the ISR one has $Q^2_{ISR}=(-p^2+m_0^2)$ with a space-like virtuality ($p^2<0$). In both cases $Q^2$ values are positively defined. 
...
\subsubsection{Various contributions to hadron-hadron cross-section}
...
Once a event has been classified as diffractive it is possible to distinguish between three possible classes of diffractive events: double-diffractive (DD), single-diffractive (SD) and central-diffractive (CD). 
...
\label{sec:Beam Beam Remnants and primordial kT in PYTHIA8}

What is left after that the $p_T$ evolution is ended?
The evolution in $p_T$ can create an arbitrary complicated final state. 
This final state contains contributions from the scattered and unscattered partons that don't enter the $p_T$ evolution. The last ones are the so-called \textit{Beam Beam Remnants} (BBR). 
...
The last important step at parton level is the \textit{color reconnection} (CR). CR is motivated by the fact that MPI leads to different color strings. In the previous steps, the planar limit of the QCD was assumed where $N_c\rightarrow\infty$. Now, moving to real case where $N_c=3$  during the development of the parton shower, partons are
...
The next chapter will focus on the study of the underlying event and so on all the soft events related to a hard scattering in a hadron-hadron collision with the description of the analyses used in our tune and the effects of some of these \textsc{pythia}8 parameters on these observables distributions. 
