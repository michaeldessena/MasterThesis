...
\end{equation}
where $h(p)$ is a vector of functions. Each function gives the value of the $i^{th}$ bin based on the  parameters values. In this phase this functions vector has been substituted by the surrogate one $\overline{h}(p)$.  
...
\end{equation}
where $sigma^2$ is the vector of the squared error on each bin.  
