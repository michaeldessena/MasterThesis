...
\noindent These parameters are the ones that govern the number of MPI and the amount of Color Reconnections\footnote{Note that the parameters related to the simulation of the hadronization and beam remnant are set to the values of the Monash tune}. These CP* tunes want to be a general (multi-purpose) tunes for UE and minimum bias observables described above.
...
On the top section of \tableRef{fig:CPtunes1} and \tableRef{fig:CPtunes2} are reported the values for the \texttt{pythia8} parameters used in the CP tunes and, on the bottom, the five parameters resulting from the tune.
\begin{table}[!htb]
	\centering
